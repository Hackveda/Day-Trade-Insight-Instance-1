...
if(isset($_SESSION["username"]) && strcmp($_SESSION["username"], "") != 0){
    ...
    $sql2 = "SELECT * from orders where `Username` like '".$username."' and `Recommend` like '".$symbol."' and BuyOpStatus like 'True'";
            
            $result2 = $con1->query($sql2);
            
            if($result2->num_rows > 0){
                while($row = $result2->fetch_assoc()){
                    $hold_op_qty = $row["BuyOpQty"];
                }
            }else{
                // Do nothing
            }
            
    $margin = $margin - ($hold_op_qty * $price);  
    ...
    if($margin < $funds){
        ...
        $sql1 = "INSERT into orders (`Recommend`, `SellOpPrice`, `SellOpQty`, `SellOpDate`, `Username`) VALUES('".$symbol."', '".$price."', '".$qty."', '".$dateTime."', '".$username."')";
